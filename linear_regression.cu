...
#define THREADS_PER_BLOCK 256
        ...
        __global__ void
        compute_partial_sums(float *x, float *y, float *x_sum, float *y_sum, float *xy_sum, float *xx_sum, int n)
{
...
int main()
{
    ...
    cudaMemset(x_sum_d, 0, sizeof(float));
    cudaMemset(y_sum_d, 0, sizeof(float));
    cudaMemset(xy_sum_d, 0, sizeof(float));
    cudaMemset(xx_sum_d, 0, sizeof(float));
    ...
    std::cout << "Sample Prediction x = 10: y = " << (w_gpu * 10 + b_gpu) << std::endl;
    std::cout << "Sample Prediction x = 5: y = " << (w_gpu * 5 + b_gpu) << std::endl;
    std::cout << "Sample Prediction x = 1: y = " << (w_gpu * 1 + b_gpu) << std::endl;
    std::cout << "Sample Prediction x = 0: y = " << (w_gpu * 0 + b_gpu) << std::endl;
    // Cleanup
    cudaFree(x_d);
    cudaFree(y_d);
    cudaFree(x_sum_d);
